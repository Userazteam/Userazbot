...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @Userazbot - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @Userazbot - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/Userazteam/Userazbot.git")

CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

DB_URI = os.environ.get("DATABASE_URL", "sqlite:///useraz.db")
...
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@Userazbot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@Userazbot Paketi")

OTOMATIK_KATILMA = sb(os.environ.get("OTOMATIK_KATILMA", "True")) 

PATTERNS = os.environ.get("PATTERNS", ".;!,")
WHITELIST = get('https://gitlab.com/codeaz/user-az/-/raw/master/whitelist.json').json()
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@Userazbot"))
            bot(JoinChannelRequest("@Userazsup"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən` @Userazbot`! Mən sahibimə kömək etmək üçün burdayam, sənə kömək edə bilmərəm. Ama sən də bir Userazbot işlədə bilərsən; Kanala bax` @Userazbot')
            else:
                await event.reply(f'`Tengri save Turks! USER-AZ working... 🐺`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@Userazbot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Sadəcə .alive komutu yoxlayın",
                    text=f"[USER-AZ](https://t.me/Userazbot)** Userbot super işləyir⚡️**\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Səhifə:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
...
Hesabınızı bota çevirə və istifadə edə bilərsiniz. Unutmayın, başqasının botunu idarə edə bilməzsiniz! Bütün quraşdırma detalları aşağıdakı GitHub adresindən izah olunur.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/Userazbot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/Userazsup")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Userazteam/Userazbot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Mesajlarımı düzəltməyə çalışmayın! Şəxsi hesabınıza @Userazbot qurun.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"[USER-AZ](https://t.me/Userazbot)** Userbot super işləyir⚡️**\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
